// ========== main_debug.ino (Debug Version) ==========
// Modified version of main.ino with LCD functionality disabled for debugging
#include <Adafruit_NeoPixel.h>
#include <WiFi.h>
#include <WebServer.h>
#include <HTTPClient.h>
#include <ArduinoJson.h>
// LCD disabled for debugging
...
void setup() {
  Serial.begin(115200);
  Serial.println("Starting SafetyAir Monitor (Debug Version)");
  ...
  if (apMode) {
    Serial.println("Device in AP mode - connect to WiFi network 'SafetyAir-Config' with password 'password'");
    Serial.println("Then access web interface at 192.168.4.1");
  } else {
    Serial.print("Device connected to WiFi - access web interface at ");
...
void readSensors() {
  ...
  // LCD update disabled for debugging
  // updateLCD(localAQ, gasDetected);
  
  // Print sensor values to serial instead
  Serial.print("Local AQ: ");
  Serial.print(localAQ);
...
void fetchCityAQI() {
  ...
  if (code == 200) {
    ...
    cityAQ = doc["data"]["aqi"];
    Serial.print("City AQI: ");
    Serial.println(cityAQ);
  }
  http.end();
}
...
// === Update LCD with AQI and temp (disabled) ===
void updateLCD(int aq, bool gasDetected) {
  // Function kept for compatibility but disabled
  // LCD disabled for debugging
}
